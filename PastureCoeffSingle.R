...
## Hannah White 2.10.2019

## Graphics for all best performing models for pasture-dominated hectads
...
source('E:\\Postdoc Grassland Resilience\\MODIS R code\\Resilience\\Analyses\\Reanalysis_Method2\\GLS_pasture_stability2sd0_1deg.R')
...
var.landclim.ci <- data.frame(intervals(m.var.landclim)$coef)
var.landclim.df <- data.frame(covariate = rownames(var.landclim.ci), var.landclim.ci, row.names = NULL)
names(var.landclim.df)[3] <- 'est'
var.landclim.df <- var.landclim.df[-1,]
var.landclim.df$Model <- rep('landclim', length(var.landclim.df$covariate))

var.landclim.df$covariate <- factor(var.landclim.df$covariate, as.character(var.landclim.df$covariate))


var.df <- rbind(var.full.df, var.clim.df, var.divclim.df, var.landclim.df)

var.df$Model <- factor(var.df$Model, levels = c('climate', 'divclim', 'landclim', 'full'))
...
p.var <- p.var + scale_x_discrete(breaks = c("nat.fres", "landscape.het",
                                            ...
                                            labels = c("SR", "Hetero",
                                                       "Var \ntemp", "Var \nprecip",
                                                       "Ext \ntemp", "Ext \nprecip"))
p.var <- p.var + scale_colour_manual(values = c('#481567FF', '#2D708EFF', '#3CBB75FF', '#B8DE29FF'))
...
p.resist <- p.resist + scale_x_discrete(breaks = c("nat.fres", "landscape.het",
                                  ...
                                  labels = c("SR", "Hetero",
                                             "Var \ntemp", "Var \nprecip",
                                             "Ext \ntemp", "Ext \nprecip"))
p.resist <- p.resist + scale_colour_manual(values = c('#481567FF', '#2D708EFF', '#3CBB75FF'))
...
rec.df <- rbind(rec.full.df, rec.clim.df, rec.divclim.df, rec.landclim.df)

rec.df$Model <- factor(rec.df$Model, levels = c('climate', 'divclim', 'landclim', 'full'))
...
p.rec <- p.rec + scale_x_discrete(breaks = c("nat.fres", "landscape.het",
                                        ...
                                        labels = c("SR", "Hetero",
                                                   "Var \ntemp", "Var \nprecip",
                                                   "Ext \ntemp", "Ext \nprecip"))
p.rec <- p.rec + scale_colour_manual(values = c('#481567FF', '#2D708EFF', '#3CBB75FF', '#B8DE29FF'))
...
p.recrate <- p.recrate + scale_x_discrete(breaks = c("nat.fres", "landscape.het",
                                  ...
                                  labels = c("SR", "Hetero",
                                             "Var \ntemp", "Var \nprecip",
                                             "Ext \ntemp", "Ext \nprecip"))
p.recrate <- p.recrate + scale_colour_manual(values = c('#481567FF', '#2D708EFF', '#3CBB75FF', '#B8DE29FF'))
p.recrate <- p.recrate + theme_bw() + theme(axis.text = element_text(size = 18, face = 'bold'), axis.title = element_blank(),
                                    legend.text = element_text(size = 18), legend.title = element_text(size = 18))

tiff('E:\\Postdoc Grassland Resilience\\Graphics\\Diversity-Resilience\\Paper\\PastureCoefsCol2.tiff',
