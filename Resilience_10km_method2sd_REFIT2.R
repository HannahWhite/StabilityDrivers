...
### Edited 23.08.2021 to make resistance into magnitude of anomalies (just absolute value)


### This is a development of previous code where now, 
### large perturbations in the anomalies are found and 180 days each side are excluded
### Additionally, these are now calculated for the 10 x 10 km 
### squares which match the vascular plant data which have been refitted correctly thte secodn time

rm(list=ls())

## read in anomaly data

load('D:\\Postdoc Grassland Resilience\\MODIS6\\Anomalies_18.02.2019\\refit2.anomaly10km.RData')
load('D:\\Postdoc Grassland Resilience\\MODIS6\\Anomalies_18.02.2019\\refit2.anomaly10km.cent.RData')
...
for (kk in 1:777){ #begins moving window at each separate time point
      ...
      if (event.length > 0){
        for (mm in 1:event.length){
          ...
          if (start<=733) {
            ...
            return.time <- which(mov.avg[ii, jj, (start+1):777]>=0)[1] # calculates recovery time in units of time windows
...
write.csv(stability10km, 'D:\\Postdoc Grassland Resilience\\MODIS6\\Resilience\\REFIT2stability10km2sd.csv', row.names = FALSE)
